...
class Animal:
    ...
    def setGenotype(self, genotype):
        ...
        query = "UPDATE `ANIMAL` SET `GENOTYPE`='{}' WHERE `ID`='{}';".format(
            genotype, self.baseId
        )
    ...
    def __str__(self):
        return "Animal Id:{id} Name:{name} RFID:{rfid} Genotype:{genotype} User1:{user1}".format(
            id=self.baseId,
            rfid=self.RFID,
            name=self.name,
    ...
    def loadDetection(self, start=None, end=None, lightLoad=False):
        ...
        query = ""
        if lightLoad == True:
            query = "SELECT FRAMENUMBER, MASS_X, MASS_Y FROM DETECTION WHERE ANIMALID={}".format(
                self.baseId
            )
        else:
    ...
    def filterDetectionByInstantSpeed(self, minSpeed, maxSpeed):
        ...
        for key in sorted(self.detectionDictionnary.keys()):
            ...
            speed = (
                math.hypot(a.massX - b.massX, a.massY - b.massY)
                * scaleFactor
                / (1 / 30)
            )
        ...
        print(
            "Filtering Instant speed min:",
            minSpeed,
            "max:",
            maxSpeed,
            "number of detection removed:",
            nbRemoved,
        )
    ...
    def getDistanceSpecZone(
        self, tmin=0, tmax=None, xa=None, ya=None, xb=None, yb=None
    ):
        ...
        for key in keyList:
            ...
            if (
                math.hypot(a.massX - b.massX, a.massY - b.massY) > 85.5
            ):  # if the distance calculated between two frames is too large, discard
                continue
    ...
    def getDistanceTo(self, t, animalB):
        ...
        if (
            math.hypot(
                self.detectionDictionnary[t].massX
                - animalB.detectionDictionnary[t].massX,
                self.detectionDictionnary[t].massY
                - animalB.detectionDictionnary[t].massY,
            )
            > 71 * 57 / 10
    ...
    def getDistanceToPoint(self, t, xPoint, yPoint):
        ...
        if (
            math.hypot(
                self.detectionDictionnary[t].massX - xPoint,
    ...
    def getSpeed(self, t):
        ...
        speed = (
            math.hypot(a.massX - b.massX, a.massY - b.massY) * scaleFactor / (2 / 30)
        )
    ...
    def getSap(self, tmin=0, tmax=None, xa=None, ya=None, xb=None, yb=None):
        ...
        for key in keyList:
            ...
            if (
                detection.massX < xa
                or detection.massX > xb
                or detection.massY < ya
                or detection.massY > yb
                or detection.massZ > self.medianBodyHeight
            ):
                # print ( 2 )
                continue
            ...
            if (
                detection.getBodySize() >= self.bodyThreshold
                and speed < SPEED_THRESHOLD_LOW
    ...
    def getCountFramesSpecZone(
        ...
        for key in keyList:
            ...
            if (
                detection.massX < x1
                or detection.massX > x2
                or detection.massY < y1
                or detection.massY > y2
            ):
                # print ( 2 )
                continue
    ...
    def getBinaryDetectionMask(self, t):
        ...
        """
        query = "SELECT DATA FROM DETECTION WHERE ANIMALID={} AND FRAMENUMBER={}".format(
            self.baseId, t
        )
...
class AnimalPool:
    ...
    def loadDetection(self, start=None, end=None, lightLoad=False):
        ...
        for animal in self.animalDictionnary.keys():
            self.animalDictionnary[animal].loadDetection(
                start=start, end=end, lightLoad=lightLoad
            )

    def filterDetectionByInstantSpeed(self, minSpeed, maxSpeed):
        for animal in self.animalDictionnary.keys():
            self.animalDictionnary[animal].filterDetectionByInstantSpeed(
                minSpeed, maxSpeed
            )
    ...
    def filterDetectionByEventTimeLine(self, eventTimeLineVoc):
        for animal in self.animalDictionnary.keys():
            self.animalDictionnary[animal].filterDetectionByEventTimeLine(
                eventTimeLineVoc
            )
    ...
    def frameToTimeTicker(self, x, pos):
        ...
        experimentTime = "D{0} - {1:02d}:{2:02d}".format(
            int(vals[0]) + 1, int(vals[1]), int(vals[2])
        )
        ...
        if x > 0:
            ...
            if datetime != None:
                realTime = "\n" + getDatetimeFromFrame(self.conn, x).strftime(
                    "%d-%b-%Y %H:%M:%S"
                )
    ...
    def plotSensorData(
        ...
        for nightEvent in nightTimeLine.eventList:
            ...
            ax.text(
                (nightEvent.startFrame + nightEvent.endFrame) / 2,
                mean + std,
                "dark phase",
                fontsize=20,
                ha="center",
        ...
        if autoNight:
            ...
            ax.text(
                0, mean, "auto night threshold", fontsize=20, ha="left", va="bottom"
            )
    ...
    def plotNight(self, show=True, saveFile=None):
        ...
        for nightEvent in nightTimeLine.eventList:
            ...
            ax.axvspan(
                nightEvent.startFrame, nightEvent.endFrame, alpha=0.1, color="black"
            )
            ax.text(
                (nightEvent.startFrame + nightEvent.endFrame) / 2,
                0.5,
    ...
    def buildSensorData(self, file, show=False):
        ...
        self.plotSensorData(
            sensor="TEMPERATURE",
            minValue=10,
            saveFile=file + "_log_temperature.pdf",
            show=show,
        )
        self.plotSensorData(
            sensor="SOUND", saveFile=file + "_log_sound level.pdf", show=show
        )
        self.plotSensorData(
            sensor="HUMIDITY",
            minValue=5,
            saveFile=file + "_log_humidity.pdf",
            show=show,
        )
        self.plotSensorData(
            sensor="LIGHTVISIBLE",
            minValue=40,
            saveFile=file + "_log_light visible.pdf",
            show=show,
        )
        self.plotSensorData(
            sensor="LIGHTVISIBLEANDIR",
            minValue=50,
            saveFile=file + "_log_light visible and infra.pdf",
            show=show,
        )
    ...
    def plotTrajectory(
        ...
        for animal in self.getAnimalList():
            ...
            if scatter == True:
                ...
                legendList.append(
                    mpatches.Patch(color=animal.getColor(), label=animal.RFID)
                )
            else:
                axis.plot(
                    xList,
    ...
    def getParticleDictionnary(self, start, end):
        ...
        query = "SELECT FRAMENUMBER, NUMPARTICLE FROM FRAME WHERE FRAMENUMBER>={} AND FRAMENUMBER<={}".format(
            start, end
        )
    ...
    def getDetectionTable(self):
        ...
        * adds column to indicate detections in center region
        ...
        df["x_cm"] = (
            (df.x - cornerCoordinates50x50Area[0][0])
            / (cornerCoordinates50x50Area[1][0] - cornerCoordinates50x50Area[0][0])
            * ARENA_SIZE
        )
        df["y_cm"] = (
            (df.y - cornerCoordinates50x50Area[1][1])
            / (cornerCoordinates50x50Area[2][1] - cornerCoordinates50x50Area[1][1])
            * ARENA_SIZE
        )

        df[f"in_arena_center"] = (
            (df["x_cm"] > CENTER_MARGIN)
    ...
    def getAllEventsTable(self):
        ...
        event_table = pd.concat(
            [
                self.getSingleEventTable(event_name)
