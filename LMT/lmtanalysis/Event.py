...
class Event:
    ...
    def __init__(
        self, startFrame, endFrame, baseId=None, metadata=None, description=None
    ):
    ...
    def numberOfFrameToEvent(self, eventCandidate):
        ...
        return min(
            abs(self.startFrame - eventCandidate.endFrame),
            abs(self.endFrame - eventCandidate.startFrame),
        )

    def __str__(self):
        return (
            "Event\tstart:\t" + str(self.startFrame) + "\tend:\t" + str(self.endFrame)
        )
...
class EventTimeLine:
    ...
    def __init__(
        ...
        if loadEvent == False:
            print(
                "Event "
                + str(eventName)
                + " created. eventNameWithId = "
                + str(self.eventNameWithId)
                + " loadEvent: False"
            )
            return
        ...
        if (
        ...
        ):  # load events without using the rebuild dictionnary.
            ...
            for row in all_rows:
                ...
                self.eventList.append(
                    Event(
                        start, end, metadata=metadata, baseId=row[0], description=row[2]
                    )
                )
    ...
    def getClosestEventFromFrame(
        self, frameNumber, optimizeAssumingOrderedList=False, constraint=None
    ):
    ...
    def printEventList(self):
        print(
            "Event list of {} / {} / nbEvent: {}".format(
                self.eventName, self.eventNameWithId, len(self.eventList)
            )
        )
        i = 0
        for event in self.eventList:
    ...
    def plotEventDurationDistributionHist(self, nbBin=10):
        ...
        plt.figtext(
            0.5, 0.85, "Number of bin: {}".format(nbBin), fontsize=10, ha="center"
        )
        plt.show()
    ...
    def plotEventDurationDistributionBar(self, minDuration=None, maxDuration=None):
        ...
        plt.figtext(
            0.5,
            0.85,
            "Number of events: {} (minDuration={}, maxDuration={})".format(
                len(data), minDuration, maxDuration
            ),
            fontsize=10,
            ha="center",
        )
    ...
    def plotTimeLine(self):
        ...
        ax.text(
            0,
            1.01,
            "TimeLine of {}".format(self.eventNameWithId),
            fontsize=10,
            ha="left",
        )
        if longestEvent != None:
            ax.annotate(
                "Longest event ({})".format(longestEvent.duration()),
                xy=(longestEvent.startFrame, 1),
                xytext=(longestEvent.startFrame, 0.95),
    ...
    def endRebuildEventTimeLine(self, connection, deleteExistingEvent=False):
        ...
        if deleteExistingEvent:
            ...
            deleteEventTimeLineInBase(
                connection, self.eventName, self.idA, self.idB, self.idC, self.idD
            )
        else:
    ...
    def getLengthDistanceWithTimeLine(self, timeLineCandidate):
        ...
        chrono = Chronometer(
            "getLengthDistanceWithTimeLine " + timeLineCandidate.eventName
        )
        ...
        for event in self.eventList:
            ...
            for eventCandidate in mergedTimeLine.eventList:
                ...
                if event.overlapEvent(eventCandidate):
                    ...
                    nbMatch += 1
                    meanEventT = (
                        event.startFrame + (event.endFrame - event.startFrame) / 2
                    )
                    ...
                    if foundEvent != None:
                        ...
                        if foundEvent.duration() > event.duration() * 2:
                            ...
                            try:
                                a = (
                                    meanEventT - foundEvent.startFrame
                                ) / foundEvent.duration()
                                a = int(a * 100)
...
def deleteEventTimeLineInBase(
    connection, eventName, idA=None, idB=None, idC=None, idD=None
):
    """
    ...
    print(
        "Deleting event {} (idA:{},idB:{},idC:{},idD:{}) from base...".format(
            eventName, idA, idB, idC, idD
        )
    )
    ...
    for timeLine in timeLineList:
        ...
        if timeLine != None:
            ...
            plt.text(
                minValue,
                1.0 + yOffset,
                "{}   ".format(timeLine.eventNameWithId),
                fontsize=9,
                ha="right",
                va="center",
                label="test",
...
class TestEventTimeLine(unittest.TestCase):
    ...
    def test_PuntualEvent5(self):
        ...
        myEventTimeLine.printEventList()
        result = (
            myEventTimeLine.getMinT() == 50
            and myEventTimeLine.getMaxT() == 400
            and len(myEventTimeLine.eventList) == 1
        )
        self.assertEqual(result, True)
    ...
    def test_PuntualEvent6(self):
        ...
        myEventTimeLine.printEventList()
        result = (
            myEventTimeLine.getMinT() == 100
            and myEventTimeLine.getMaxT() == 300
            and len(myEventTimeLine.eventList) == 1
        )
        self.assertEqual(result, True)
